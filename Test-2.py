...
def menu(message):
    ...
    send = bot.send_message(message.chat.id, send_mess, parse_mode='html', reply_markup=markup)
    bot.register_next_step_handler(send, menu1)
...
def menu1(message):
    if message.data == 'Currency':
        ...
        send =bot.edit_message_text(chat_id=message.chat.id, message_id=message.message_id, text="Какая Вас интересует валюта?", parse_mode='html', reply_markup=markup)
        bot.register_next_step_handler(send, menu2)
    elif message.data == 'Shares':
        ...
        btn3 = types.InlineKeyboardButton('Назад', callback_data='BACK')
        markup.add(btn1, btn2, btn3)
        send = bot.edit_message_text(chat_id=message.chat.id, message_id=message.message_id, text="Какие Вас интересует Ценные бумаги?", parse_mode='html', reply_markup=markup)
        bot.register_next_step_handler(send, menu2)
    elif message.data == 'Crypt':
        markup = types.InlineKeyboardMarkup(row_width=2)
        ...
        btn3 = types.InlineKeyboardButton('Назад', callback_data='BACK')
        markup.add(btn1, btn2,btn3)
        send = bot.edit_message_text(chat_id=message.message.chat.id, message_id=message.message.message_id, text="Какие Вас интересует криптовалюта?", parse_mode='html', reply_markup=markup)
        bot.register_next_step_handler(send, menu2)
    elif message.data == 'Feedback':
        markup = types.InlineKeyboardMarkup(row_width=1)
        ...
        send = bot.edit_message_text(chat_id=message.message.chat.id, message_id=message.message.message_id, text="Оставьте свой отзыв или предложение", reply_markup=markup)
        bot.register_next_step_handler(send, menu2)
